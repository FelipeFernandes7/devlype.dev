  ...
  return (
    <header className=" z-30 mt-3 flex min-h-[3.75rem] w-full flex-wrap items-center text-xs text-white md:mt-0">
                  ...
                  >
                    {isHovered && (
                      <motion.span
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="absolute left-0 right-0 top-[7px] -z-10 rounded-lg bg-[#111827] p-5"
                      />
                    )}
                    {page}
                  </motion.span>
      ...
      <aside className="order-2 ml-auto mr-3 flex items-center">
        <div className="flex cursor-pointer items-center justify-center rounded-lg bg-transparent p-2 ease-in-out hover:bg-hover text-3xl font-bold hover:opacity-60 transition-all duration-300">
          ⌘
        </div>
      </aside>
    </header>
